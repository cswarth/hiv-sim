...
pwscore <- Curry(pwalign,scoreOnly=TRUE)
...
get_sequences <- function(dir) {
    c(founder=founder,
      dist.beast=beast.sequence(dir),
      dist.pcodon=prank.sequence(dir,'codon'),
      dist.pdna=prank.sequence(dir, 'dna'),
      dist.unguided=prank.sequence(dir, 'unguided'),
      dist.consensus=consensus.sequence(dir))
}
...
distances.pwscore <- function(dir) {
    get_sequences(dir) %>%
    sapply(function(s) pwscore(founder,s)) %>% t()
}
