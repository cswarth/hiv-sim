...
get_sequences <- function(dir) {
    c(strict=beast.sequence(file.path(dir,"strict")),
      relaxed=beast.sequence(file.path(dir,"relaxed")),
      consensus=consensus.sequence(dir))
}

# calculate how many possible combinations of factors
ttotal <- prod(sapply(df, function(c) switch(class(c), factor=length(levels(c)),logical=2,1)))
cat(sprintf('Possible combinations of factors: %d\n', ttotal))
cat(sprintf('Number of samples to process: %d\n', nrow(df)))
if (ttotal != nrow(df)) {
    warn('These two numbers should be the same!')
} else {
    cat('Good, these numbers are the same.\n')
}
